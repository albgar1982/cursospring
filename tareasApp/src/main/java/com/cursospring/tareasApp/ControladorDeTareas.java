...
public class ControladorDeTareas {
	...
	public String mostrarFormularioTarea(ModelMap modelMap) {
		...
		Tarea tarea = new Tarea(0, nombreUsuario, "", LocalDate.now().plusYears(1), false); //Creamos un objeto Tarea vacío para que el formulario de la vista tareaNueva.jsp pueda enlazarlo con sus campos (mapear el formulario de la vista a esta tarea)
		modelMap.put("tareita", tarea); //Añadimos el objeto Tarea al ModelMap para que la vista pueda acceder a él. El nombre que usamos aquí es el que usamos en el form:form de la vista tareaNueva.jsp
		return "tareaNueva";
	}
	...
	*/
	...
	@PostMapping(value = "nueva-tarea")
	public String enviarFormularioTarea(ModelMap modelMap,@ModelAttribute @Valid Tarea tareita //Recogemos el objeto Tarea que Spring ha creado automáticamente con los datos del formulario (enlazados desde la vista tareaNueva usando modelAttribute="tareita" en la etiqueta form:form del formulario en la vista. Hay que poner @Valid para que se apliquen las validaciones que están en su clase (se aplican en el front)
			,BindingResult result) { //Este parámetro captura los errores de validación que se produzcan
		
		if(result.hasErrors()) { //Si hay errores de validación, volvemos a la vista tareaNueva.jsp
			return "tareaNueva";
		}
		String nombreUsuario = (String) modelMap.get("nom");
		servicioDeTareas.ponTarea(nombreUsuario, tareita.getDescripcion(),LocalDate.now().plusYears(1), false);
		return "redirect:tareas"; //Con redirect llamamos al ENDPOINT que queramos
	}
